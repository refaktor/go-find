...
func (p *mountPredicate) Match(_ string, path string) (bool, error) {
	fsType := getFileSystemType(path)
	for _, t := range p.fsType {
		for _, fst := range fsType {
			if t == fst {
				return true, nil
			}
		}
	}
	return false, PredicateError{errType: ErrorFSType, errMessage: "no such filesystem"}
}
